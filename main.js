...
function modifyOrDltItem(e) {
  ...
  if (e.target.classList.contains("delete-product")) {
    ...
    const updateBtn = `<button type='submit' class ='btn mt-3 btn-block btn-outline-success update-product'>Update</button>`;
...
function addDataToUI(productList) {
  if (productData.length > 0) {
    productList.forEach((product) => {
      ...
      li.className = "list-group-item list-group-item-danger collection-item";
      li.id = `product-${id}`;
